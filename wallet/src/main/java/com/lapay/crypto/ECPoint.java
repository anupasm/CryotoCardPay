...
public class ECPoint {
    ...
    /**
     * Creates new ECPoint object for provided {@code curve}. Random initial point
     * value is generated.
    ...
    /**
     * Set this point value (parameter W) from array with value encoded as per ANSI
     * X9.62.
     * The uncompressed form is always supported. If underlying native JavaCard
     * implementation
     * of {@code ECPublickKey} supports compressed points, then this method accepts
     * also compressed points.
     * 
     ...
     * before return, so change of returned object will also change this point
     * value.
     * 
    ...
    public short getY(byte[] buffer, short offset) {
        ech.lock(ech.uncompressed_point_arr1);
        thePoint.getW(ech.uncompressed_point_arr1, (short) 0);
        Util.arrayCopyNonAtomic(ech.uncompressed_point_arr1, (short) (1 + this.theCurve.COORD_SIZE), buffer, offset,
                this.theCurve.COORD_SIZE);
        ech.unlock(ech.uncompressed_point_arr1);
        return this.theCurve.COORD_SIZE;
    }
    ...
    public void swDouble() {
        ...
        pY.lock();
        pY.from_byte_array(theCurve.COORD_SIZE, (short) 0, ech.uncompressed_point_arr1,
                (short) (1 + theCurve.COORD_SIZE));
    ...
    /**
     * Adds this (P) and provided (Q) point. Stores a resulting value into this
     * point.
     * 
    ...
    public void add(ECPoint other) {
        ...
        ech.fnc_add_y_p.from_byte_array(this.theCurve.COORD_SIZE, (short) 0, ech.uncompressed_point_arr1,
                (short) (1 + this.theCurve.COORD_SIZE));
        ech.unlock(ech.uncompressed_point_arr1);
        ...
        if (samePoint) {
            ...
            ech.fnc_add_x_q.from_byte_array(other.theCurve.COORD_SIZE, (short) 0, ech.uncompressed_point_arr1,
                    (short) 1);
            ech.fnc_add_nominator.set_size(this.theCurve.COORD_SIZE);
            ech.fnc_add_nominator.from_byte_array(this.theCurve.COORD_SIZE, (short) 0, ech.uncompressed_point_arr1,
                    (short) (1 + this.theCurve.COORD_SIZE));
            ech.unlock(ech.uncompressed_point_arr1);
        ...
        // If x_r.length() and y_r.length() is smaller than this.TheCurve.COORD_SIZE due
        // to leading zeroes which were shrinked before, then we must add these back
        ech.fnc_add_x_r.prepend_zeros(this.theCurve.COORD_SIZE, ech.uncompressed_point_arr1, (short) 1);
        ech.fnc_add_x_r.unlock();
        ech.fnc_add_y_r.prepend_zeros(this.theCurve.COORD_SIZE, ech.uncompressed_point_arr1,
                (short) (1 + this.theCurve.COORD_SIZE));
    ...
    /**
     * Multiply value of this point by provided scalar. Stores the result into this
     * point.
     * 
     * @param scalar value of scalar for multiplication
     */
    ...
    /**
     * Multiply value of this point by provided scalar using XY key agreement.
     * Stores the result into this point.
     * 
     * @param scalar value of scalar for multiplication
     */
    ...
    /**
     * Multiply value of this point by provided scalar using X-only key agreement.
     * Stores the result into this point.
     * 
     * @param scalar value of scalar for multiplication
     */
    private void multiplication_x(Bignat scalar) {
        ...
        ech.fnc_multiplication_y1.prepend_zeros(this.theCurve.COORD_SIZE, ech.uncompressed_point_arr1,
                (short) (1 + theCurve.COORD_SIZE));
        ...
        if (!SignVerifyECDSA(this.theCurve.bignatAsPrivateKey(scalar), this.asPublicKey(),
                this.ech.fnc_SignVerifyECDSA_signEngine, ech.fnc_multiplication_resultArray)) { // If verification
                                                                                                // fails, then pick the
                                                                                                // <x, y_2>
    ...
    private short multiplication_x_KA(Bignat scalar, byte[] outBuffer, short outBufferOffset) {
        // NOTE: potential problem on real cards (j2e) - when small scalar is used
        // (e.g., Bignat.TWO), operation sometimes freezes
        ...
        // Return always length of whole coordinate X instead of len - some real cards
        // returns shorter value equal to SHA-1 output size although PLAIN results is
        // filled into buffer (GD60)
        return this.theCurve.COORD_SIZE;
    }
    ...
    public void negate() {
        ...
        // Operation will dump point into uncompressed_point_arr, negate Y and restore
        // back
        ...
        ech.fnc_negate_yBN.from_byte_array(this.theCurve.COORD_SIZE, (short) 0, ech.uncompressed_point_arr1,
                (short) (1 + this.theCurve.COORD_SIZE));
        ...
        ech.fnc_negate_yBN.prepend_zeros(this.theCurve.COORD_SIZE, ech.uncompressed_point_arr1,
                (short) (1 + this.theCurve.COORD_SIZE));
    ...
    /**
     * Restore point from X coordinate. Stores one of the two results into this
     * point.
     *
    ...
    private void from_x(Bignat x) {
        ...
        ech.fnc_from_x_y.prepend_zeros(this.theCurve.COORD_SIZE, ech.uncompressed_point_arr1,
                (short) (1 + theCurve.COORD_SIZE));
    ...
    /**
     * Compares this and provided point for equality. The comparison is made using
     * hash of both values to prevent leak of position of mismatching byte.
     * 
     * @param other second point for comparison
     * @return true if both point are exactly equal (same length, same value), false
     *         otherwise
     */
    public boolean isEqual(ECPoint other) {
        ...
        if (this.length() != other.length()) {
            ...
            // This way, offset of first mismatching byte is not leaked via timing
            // side-channel.
            // Additionally, only single array is required for storage of plain point values
            // thus saving some RAM.
            ech.lock(ech.uncompressed_point_arr1);
            ech.lock(ech.fnc_isEqual_hashArray);
            // ech.lock(ech.fnc_isEqual_hashEngine);
            short len = this.getW(ech.uncompressed_point_arr1, (short) 0);
            ech.fnc_isEqual_hashEngine.doFinal(ech.uncompressed_point_arr1, (short) 0, len, ech.fnc_isEqual_hashArray,
                    (short) 0);
            len = other.getW(ech.uncompressed_point_arr1, (short) 0);
            len = ech.fnc_isEqual_hashEngine.doFinal(ech.uncompressed_point_arr1, (short) 0, len,
                    ech.uncompressed_point_arr1, (short) 0);
            bResult = Util.arrayCompare(ech.fnc_isEqual_hashArray, (short) 0, ech.uncompressed_point_arr1, (short) 0,
                    len) == 0;
    ...
    public static boolean SignVerifyECDSA(ECPrivateKey privateKey, ECPublicKey publicKey, Signature signEngine,
            byte[] tmpSignArray) {
     ...
     * @return length of X coordinate (in bytes)
     */
    public short encode(byte[] buffer, short offset) {
        if (is_y_even()) {
        ...
        ech.lock(ech.uncompressed_point_arr1);
        thePoint.getW(ech.uncompressed_point_arr1, (short) 0);
        Util.arrayCopyNonAtomic(ech.uncompressed_point_arr1, (short) 1, buffer, (short) (offset + 1),
                this.theCurve.COORD_SIZE);
        ech.unlock(ech.uncompressed_point_arr1);
        return this.theCurve.COORD_SIZE;
    }
